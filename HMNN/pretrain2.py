...
if __name__ == "__main__":
    ...
    parser.add_argument('--water_model_save_path', type=str, help='')
    parser.add_argument('--air_model_save_path', type=str, help='')
    parser.add_argument('--water_select_expert_config', type=str, help='')
    parser.add_argument('--air_select_expert_config', type=str, help='')
    ...
    parser.add_argument('--gate_output_dim', type=int, default=6, help='')
    ...
    parser.add_argument('--eps', type=str, default="random", help='')
    ...
    x_train, x_test, y_train, y_test = get_train_test_data(karg.file_path, test_ratio=karg.test_ratio,seed=karg.seed,eps=karg.eps)
    ...
    model_water.load_state_dict(torch.load(water_model_save_path))
    model_air = MMoE_Model_Expert()
    model_air.load_state_dict(torch.load(air_model_save_path))
    if karg.frozen:
    ...
    print(x_test.shape)
    water_expert_represents_test = model_water(torch.Tensor(x_test))[:, water_select_expert, :]
    air_expert_represents_test = model_air(torch.Tensor(x_test))[:, air_select_expert, :]
